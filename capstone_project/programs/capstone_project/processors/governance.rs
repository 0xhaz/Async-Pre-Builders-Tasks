...
pub fn process_create_governance_proposal(
    ...
    let governance_config = vault.governance_config.as_ref().ok_or(ProgramError::InvalidAccountData)?;
...
pub fn process_cast_vote(
    ...
    if vault.vote_records.iter().any(|v| v.voter == *voter.key && v.proposal_uid == proposal_id) {
        return Err(ProgramError::InvalidAccountData);
    }

    let voting_power = 100; // Placeholder for actual voting power calculation based on token balance

    match vote_type {
        VoteType::For => proposal.for_votes += voting_power,
        VoteType::Against => proposal.again_votes += voting_power,
...
pub fn process_queue_proposal(
    ...
    let proposal_index = vault.governance_proposals.iter().position(|p| p.id == proposal_id).ok_or(ProgramError::InvalidAccountData)?;
    ...
    vault.governance_proposals[proposal_idnex].eta = clock.unix_timestamp + governance_config.timelock_delay;

    drop(vault_data);
    vault.serialize(&mut &mut vault_account.data.borrow_mut()[..])?;

    msg!("Proposal {} queued for execution", proposal_id, vault.governance_proposals[proposal_index].eta);
    Ok(())
}
...
pub fn process_execute_governance_proposal(
    ...
    let execution_threshold_reached = if total_votes > 0 {
        (proposal.for_votes * 10_000 / total_votes) >= governance_config.execution_threshold as u64
    } else {
        false
    };
